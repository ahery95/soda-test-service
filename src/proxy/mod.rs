...
use hyper::{Method, Request, Response,Error};
use hyper::client::{Client};
use futures::{future, Future,};
use std::time::Duration;


pub fn forward(req: Request<AppState>) -> http::Result<Response<()>> {
  let mut new_url = req.body().forward_url.clone();
  new_url.set_path(req.uri().path());
  new_url.set_query(req.uri().query());

  let mut client_builder = Request::builder(); // <- the max timeout we allow for Selenium commands
...
fn construct_response(resp: Response) -> Box<dyn Future<Item = Response, Error = Error>> {
  let mut client_resp = Response::build(resp.status());
